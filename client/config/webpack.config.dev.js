...
// This is the development configuration. // It is focused on developer experience and fast rebuilds. // The production configuration is different and lives in a separate file.
module.exports = {
	...
	resolve: {
		// This allows you to set a fallback for where Webpack should look for modules. We placed these paths second because we want `node_modules` to "win" if there are any conflicts. This matches Node resolution mechanism.
	...
	module: {
		...
		rules: [
			...
			},
			{
				...
				oneOf: [
					// "url" loader works like "file" loader except that it embeds assets smaller than specified limit in bytes as data URLs to avoid requests. A missing `test` is equivalent to a match.
					{
					...
					// "postcss" loader applies autoprefixer to our CSS. "css" loader resolves paths in CSS and adds assets as dependencies. "style" loader turns CSS into JS modules that inject <style> tags. In production, we use a plugin to extract that CSS to a file, but in development "style" loader enables hot editing of CSS.
					{
						test: /\.css$/,
						use: [
							require.resolve('style-loader'),
					...
					{ // Exclude `js` files to keep "css" loader working as it injects its runtime that would otherwise processed through "file" loader. Also exclude `html` and `json` extensions so they get processed by webpacks internal loaders.
